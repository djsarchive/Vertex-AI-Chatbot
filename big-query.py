...
class SimplePOCChatbot:
    ...
    def query_bigquery(self, sql_query):
        """Execute a direct SQL query and return results as DataFrame"""
        try:
    ...
    def generate_sql_from_question(self, question: str) -> pd.DataFrame:
        ...
        prompt = f"""You are a BigQuery SQL generator with conversation memory. Your task is to convert natural language questions into valid BigQuery SQL queries, considering the conversation context.
                        ...
                        5. If there is a specific word for a material, order in the user query always turn it into lowercase first
    ...
    def query_documents(self, question: str, k: int = 3):
    ...
    def chat(self, user_query):
        ...
        if df_result is not None and not df_result.empty:
            ...
            # Handle document search or fallback
            if self.documents_processed and self.vector_store:
                relevant_docs = self.query_documents(user_query)
                if relevant_docs:
                    context = "\n\n".join([doc.page_content for doc in relevant_docs])
                    conversation_history = self.get_conversation_history()
               
                    prompt = f"""
Based on the conversation history and document context, please answer the user's question:
...
Please provide a comprehensive answer that considers both the conversation context and the document information. Reference previous discussions if relevant.
"""
                    try:
                        response = self.model.generate_content(prompt)
                        final_response = response.text.strip()
                    except Exception as e:
                        final_response = "I found relevant documents but couldn't generate a response."
                else:
